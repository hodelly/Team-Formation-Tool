...
export default class AddQuestionToSurvey extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      titleValue: '', questionType: 'checkbox', answersValues: '', questionWeight: 0,
    };
  ...
  handleNext() {
    const newQAdded = (this.state.questionType === 'comment' || this.state.questionType === 'schedule')
      ? {
  ...
  render() {
    const options = (this.state.questionType === 'comment' || this.state.questionType === 'schedule') ? null : (
      <label htmlFor="options">
Options:
        <input type="text" value={this.state.answersValues} onChange={this.handleValuesChange} />
      </label>
    );
    return (
      <div>
...
Question Weight:
          <input type="range" min="-10" max="10" value={this.state.questionWeight} onChange={this.handleSliderChange} />
        </label>
